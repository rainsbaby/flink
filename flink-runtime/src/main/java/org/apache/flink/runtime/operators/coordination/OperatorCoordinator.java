...
/**
 * 运行时operator的管理者。 OperatorCoordinator运行在master上，与operator的job vertex进行关联。 通过发送operator
 * event与operator进行交互。
 *
 * <p>source/sink/。。operator的coordinator，发现并分配工作，或进行聚合、提交metadata。
 *
 * <p>A coordinator for runtime operators. The OperatorCoordinator runs on the master, associated
 * with the job vertex of the operator. It communicates with operators via sending operator events.
 *
...
public interface OperatorCoordinator extends CheckpointListener, AutoCloseable {
     ...
     *
     * <p>Takes a checkpoint of the coordinator. The checkpoint is identified by the given ID.
     *
     ...
     *
     * <p>The {@code SubtaskGateway} is the way to interact with a specific parallel instance of the
